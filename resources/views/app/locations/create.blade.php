<x-layouts.app title="Add Location">
    ...
    <div class="max-w-2xl">
        <x-ui.card class="p-6">
            <form method="POST" action="{{ route('app.locations.store') }}">
                @csrf
                @if(request('wizard'))
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <x-ui.input id="name" type="text" name="name" class="mt-1" :value="old('name')" placeholder="e.g. Downtown Branch" required autofocus />
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3" class="flex w-full rounded-md border border-border-base bg-surface-1 px-3 py-2 text-sm placeholder:text-text-tertiary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500 focus-visible:border-signal-500 mt-1" placeholder="e.g. 123 Main St, City">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
