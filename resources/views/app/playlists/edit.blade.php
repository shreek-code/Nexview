<x-layouts.app title="Edit Playlist">
    <div class="mb-8">
        <div class="flex items-center space-x-3 text-sm text-text-tertiary mb-2">
            <a href="{{ route('app.playlists.index') }}" class="hover:text-text-primary transition-colors">Playlists</a>
            <x-heroicon-o-chevron-right class="w-4 h-4" />
            <span class="text-text-primary">Edit</span>
        </div>
        <h1 class="text-2xl font-bold text-text-primary tracking-tight">Edit {{ $playlist->name }}</h1>
    </div>

    @php
        // Prepare initial items for AlpineJS
        $initialItems = $playlist->items->map(function($item) {
            return [
                'uid' => uniqid(),
    ...
    <form method="POST" action="{{ route('app.playlists.update', $playlist) }}" x-data="{
            ...
            <div class="xl:col-span-2 space-y-6">
                <x-ui.card class="p-6">
                    ...
                    <div>
                        ...
                        <x-ui.input id="name" type="text" name="name" class="mt-1" :value="old('name', $playlist->name)" required autofocus />
                        @error('name')
                            ...
                            <div class="flex items-center p-3 bg-surface-2 border border-border-subtle rounded-xl group transition-all">
                                ...
                                <div class="ml-4 flex items-center space-x-2">
                                    <div class="flex items-center">
                                        <input type="number" :name="'media['+index+'][duration]'" x-model="item.duration" min="1" class="w-20 h-9 rounded-l-md border border-r-0 border-border-base bg-surface-1 px-3 py-1 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-signal-500 z-10" required>
