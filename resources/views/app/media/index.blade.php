<x-layouts.app title="Media Library">
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-3" x-data>
            <x-ui.button @click="$dispatch('open-modal', 'upload-media-modal')">
            ...
            <x-ui.card class="overflow-hidden group hover:shadow-md transition-shadow">
                <div class="aspect-video relative bg-surface-2 overflow-hidden">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                        <form method="POST" action="{{ route('app.media.destroy', $asset) }}" class="flex gap-2">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-10 h-10 rounded-full bg-red-500/90 text-white flex items-center justify-center hover:bg-red-600 hover:scale-110 transition-all shadow-lg" onclick="return confirm('Are you sure you want to delete this media asset?');" title="Delete">
    ...
    <x-ui.modal name="upload-media-modal" maxWidth="md">
        <div class="p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="$dispatch('close-modal', 'upload-media-modal')" class="text-text-tertiary hover:text-text-primary">
                    <x-heroicon-o-x-mark class="w-5 h-5" />
                </button>
            </div>
            
            <form method="POST" action="{{ route('app.media.store') }}" enctype="multipart/form-data" x-data="{
                fileName: '',
                isDragging: false,
                handleDrop(e) {
                    ...
                    <div class="text-center">
                        ...
                        <div class="mt-4 flex text-sm leading-6 text-text-secondary justify-center">
                            <label for="file_upload" class="relative cursor-pointer rounded-md bg-transparent font-semibold text-signal-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-signal-500 focus-within:ring-offset-2 hover:text-signal-500">
                                ...
                                <input id="file_upload" name="file" type="file" class="sr-only" required accept="image/jpeg,image/png,image/gif,video/mp4,video/quicktime" @change="fileName = $event.target.files[0]?.name">
                            </label>
                            <p class="pl-1" x-show="!fileName">or drag and drop</p>
                        </div>
                        <p class="text-xs leading-5 text-text-tertiary" x-show="!fileName">PNG, JPG, GIF up to 10MB</p>
                        <p class="text-xs leading-5 text-text-tertiary" x-show="!fileName">MP4, MOV up to 100MB</p>
                        
                        <p class="mt-4 text-sm font-medium text-text-primary bg-surface-2 py-2 px-4 rounded-full inline-block" x-show="fileName" x-text="fileName"></p>
                    </div>
                </div>

                <div class="mt-6 flex justify-end space-x-3">
                    <x-ui.button type="button" variant="outline" @click="$dispatch('close-modal', 'upload-media-modal')">
                        Cancel
                    </x-ui.button>
