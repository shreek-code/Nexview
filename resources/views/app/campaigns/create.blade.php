<x-layouts.app title="Create Campaign">
    ...
    <form method="POST" action="{{ route('app.campaigns.store') }}" x-data="{
        targetType: '{{ old('target_type', 'location') }}',
        contentType: '{{ old('content_type', 'existing_playlist') }}',
        ...
        <div class="max-w-3xl space-y-6">
            ...
            <x-ui.card class="p-6">
                ...
                <div>
                    ...
                    <x-ui.input id="name" type="text" name="name" class="mt-1" :value="old('name')" placeholder="e.g. Summer Promo 2024" required autofocus />
                    @error('name')
                        ...
                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto p-1">
                            @forelse($locations as $location)
                                <label class="flex items-center space-x-3 p-3 border border-border-subtle rounded-lg hover:bg-surface-2 cursor-pointer transition-colors">
                                    <input type="checkbox" name="location_ids[]" value="{{ $location->id }}" class="rounded border-border-base text-signal-600 focus:ring-signal-500" {{ in_array($location->id, old('location_ids', [])) ? 'checked' : '' }}>
                        ...
                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto p-1">
                            @forelse($screens as $screen)
                                <label class="flex items-start space-x-3 p-3 border border-border-subtle rounded-lg hover:bg-surface-2 cursor-pointer transition-colors">
                                    <input type="checkbox" name="screen_ids[]" value="{{ $screen->id }}" class="rounded border-border-base text-signal-600 focus:ring-signal-500 mt-1" {{ in_array($screen->id, old('screen_ids', [])) ? 'checked' : '' }}>
                                    <div>
